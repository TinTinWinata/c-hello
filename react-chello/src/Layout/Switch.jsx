...
export default function SwitchToggle() {
  const { userDb, refreshPage } = useUserAuth();
  const [enabled, setEnabled] = useState(false);

  useEffect(() => {
    setEnabled(userDb.notification);
  ...
  function handleOnChange() {
    ...
    updateUserOnDatabase(userDb.userId, changes)
      .then(() => {
        toastSuccess("Success change notifications setting");
        refreshPage();
      })
      .catch((e) => {
        toastError("Failed to change notifications " + e.message);
        refreshPage();
      });
  }
